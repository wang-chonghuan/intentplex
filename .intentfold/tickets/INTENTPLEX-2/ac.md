...
## AC3 — 三页各是倒序列表 + 由条目汇总的筛选，条目图文并存

**怎么查.** 对 `/posts`、`/articles`、`/works` 各做一遍：
1. 读所有列表行的日期，断言是**非递增**序列（倒序）；
2. 断言筛选行里除「All」外至少有两个标签；
3. 断言每一行都有一个 `img` 且其 `naturalWidth > 0`（图真的加载了，不是坏图标），同时有非空
   的标题文字——这就是「图片和文字的结构」；
4. 点击筛选行里第二个标签（第一个是 All），断言列表行数变少或不变，且**剩下的每一行**都带
   该标签文字；
5. 断言筛选项来自数据而非写死：`/posts` 与 `/articles` 的标签集合不相同。

第 5 条是「自动汇总」的可判定形式——一份写死的枚举会让三页出现同一组标签。
